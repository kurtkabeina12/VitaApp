...
export default function TestSocketScreen({route}) {
  ...
  const openImage = async(Url) => {
    console.log(1)
    navigation.navigate('showImage', {Url: Url, isDarkTheme:isDarkTheme})
  }
                ...
                <View style={{
                  ...
                  {message.imageUrl && (
                    <View style={{
                      alignSelf: isOurMessage ? "flex-end" : "flex-start",
                      borderWidth:  1,
                      borderColor:isDarkTheme ?'#581c87' : "#111827",
                      borderRadius:15,
                      marginTop:10,
                      marginBottom:10,
                      marginRight: isOurMessage ? 10 : 0,
                      marginLeft: isOurMessage ? 0 : 10,
                    }}>
                      <TouchableOpacity onPress={() => {openImage(message.imageUrl)}} style={{}}>
                        <Image source={{ uri: message.imageUrl }} style={{ width:   200, height:   200, borderRadius:15 }} resizeMode='cover' />
                      </TouchableOpacity>
                      <View style={{display: "flex", flexDirection: "row", position:"absolute", bottom:0, right:0, marginRight:5}}>
                        <Text style={isDarkTheme ? styles.messageTimeDark : styles.messageTimeImage}>{timeDisplay}</Text>
                        {isOurMessage && (
                          message.source === 'local' ? (
                            isMessageReceived ? <Feather style={{marginLeft:   2, color: isDarkTheme ? "white" : "#ddfaff"}} name="check" size={12}  /> :
                              <Feather style={{marginLeft:   2, color: isDarkTheme ? "white" : "#ddfaff"}} name="loader" size={12} />
                          ) : (
                            <AntDesign style={{marginLeft:   2, color: isDarkTheme ? "white" : "#ddfaff"}} name="check" size={12} />
                          )
                        )}
                    ...
                    {/*  <TouchableOpacity onPress={() => {openImage(message.imageUrl)}}>*/}
                    {/*    <Image source={{ uri: message.imageUrl }} style={{ width: 100, height: 100 }} resizeMode='contain' />*/}
                    {/*  </TouchableOpacity>*/}
                    {/*)}*/}
                      ...
                      <View
                        style={[
                          styles.messageContainer,
                          {
                            ...
                            backgroundColor: isOurMessage ? (isDarkTheme ? "#581c87" : "#f0fdf4") : (isDarkTheme ? "#18181b" : "#f8fafc"),
                          },
                        ]}
                      >
                      <TouchableOpacity onPress={() => handleFilePress(message.msgfile, message.og_filename)}>
                        <View style={{display:'flex', flexDirection:'row', alignItems:'center'}}>
                          <View style={{borderRadius: 20, backgroundColor: isDarkTheme ? "#01591e" : '#00862b', padding: 10}}>
                            <FontAwesome name="file" size={20} color="white" />
                          </View>
                          <View style={{display:'flex', flexDirection:'column'}}>
                            <Text style={{marginLeft: 10, marginTop:5, color:isDarkTheme ? "#167c38" :"#00862b"}}>{message.og_filename}</Text>
                            <Text style={{paddingTop: 10, marginLeft: 10, color: isDarkTheme ? "#167c38" : "#00862b", fontSize:10}}> {formatBytes(message.file_size)}</Text>
                          </View>
                        </View>
                      </TouchableOpacity>
                      </View>
                    ...
                    {message.msgtext && (
                      ...
                      >
                    <Text style={ isDarkTheme ? styles.messageTextDark : styles.messageText}>{message?.msgtext}{message.text}</Text>
                    <View style={{display: "flex", flexDirection: "row"}}>
                      <Text style={isDarkTheme ? styles.messageTimeDark : styles.messageTime}>{timeDisplay}</Text>
                      {isOurMessage && (
                        message.source === 'local' ? (
                          isMessageReceived ? <Feather style={{marginLeft: 2, color: isDarkTheme ? "white" : "black"}} name="check" size={12}  /> :
                            <Feather style={{marginLeft: 2, color: isDarkTheme ? "white" : "black"}} name="loader" size={12} />
                        ) : (
                          <AntDesign style={{marginLeft: 2, color: isDarkTheme ? "white" : "black"}} name="check" size={12} />
                        )
                      )}
                      {message.edited && <Text style={styles.editedLabel}>Изменено</Text>}
                    </View>
